...
unsigned char ssd[4] = {0xE7, 0xE7, 0xE7, 0xE7};
unsigned char update[] = {0XE5, 0x08, 0x6E, 0xCC};
unsigned char product[] = {0X8F, 0x08, 0x80, 0XE9};
    ...
    while (1)
    {
        ...
        if(interrupt_flag)
        {
            ...
            if(normal_client_flag)
            {
                ...
                if((key==0x0B) && (update_save_flag==0) && (product_save_flag==0))
                    sw3_flag=!sw3_flag;
...
void can_receive_data(void)
{
    if(can_receive())
    {
        received_up_stk[0]=can_payload[5 +0];
        received_up_stk[1]=can_payload[5 +1];
        received_up_stk[2]=can_payload[5 +2];
        received_up_stk[3]=can_payload[5 +3];
        received_up_stk[4]='\0';
        received_pt_id[0]=can_payload[5 +4];
        received_pt_id[1]=can_payload[5 +5];
        received_pt_id[2]=can_payload[5 +6];
        received_pt_id[3]=can_payload[5 +7];
        received_pt_id[4]='\0';
# 384 "main.c"
        int index=0;
